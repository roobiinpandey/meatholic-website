-- Meatholic Full CMS Schema (secure) - Run in Supabase SQL Editor
...
create policy "Admin manage dishes" on public.dishes for all to authenticated
  using (public.is_admin()) with check (public.is_admin());
...
create policy "Admin manage specials" on public.specials for all to authenticated
  using (public.is_admin()) with check (public.is_admin());
